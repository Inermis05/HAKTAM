...
if __name__ == "__main__":
    ...
    # 처음 10 라운드는 신뢰도를 쌓기 위한 위장 단계
    CAMOUFLAGE_ROUNDS = 10
    print(f"--- Camouflage Attack Simulation ---")
    print(f"Attacker nodes {ATTACKER_NODES} will build trust for {CAMOUFLAGE_ROUNDS} rounds, then attack.")
    ...
    for i in range(NUM_ROUNDS):
        ...
        # 2. 공격자 노드의 투표 전략 결정
        # 위장 단계: 정직한 노드들이 가장 많이 투표한 후보(예상 승자)에게 투표
        if round_num <= CAMOUFLAGE_ROUNDS:
            if not honest_votes:
            ...
            if round_num == CAMOUFLAGE_ROUNDS:
                 print(f"\n--- Last round of camouflage. Attackers voting for {attacker_vote} to build trust. ---")
        # 공격 단계: 목표 후보에게 투표 집중
        else:
            attacker_vote = ATTACKER_CANDIDATE
            if round_num == CAMOUFLAGE_ROUNDS + 1:
                print(f"\n--- Attack phase started! Attackers are now voting for {ATTACKER_CANDIDATE}. ---")

        for node_id in ATTACKER_NODES:
            votes[node_id] = attacker_vote

        winner = sim.run_round(round_num=round_num, votes_cast=votes)
