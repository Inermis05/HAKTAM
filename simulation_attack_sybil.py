...
if __name__ == "__main__":
    ...
    # 공격자는 0, 1, 2번 체인을 완전히 장악함 (전체 노드의 30%)
    ATTACKER_CHAINS = [0, 1, 2]
    print(f"--- Sybil Attack Simulation ---")
    print(f"Attacker controls chains {ATTACKER_CHAINS} to vote for {ATTACKER_CANDIDATE}\n")
    ...
    for i in range(NUM_ROUNDS):
        ...
        for node_id in range(NUM_NODES):
            chain_id = sim.node_to_chain_map[node_id]
            # 공격자가 장악한 체인의 노드는 무조건 공격 대상에게 투표
            if chain_id in ATTACKER_CHAINS:
                votes[node_id] = ATTACKER_CANDIDATE
            # 나머지 체인의 노드는 무작위 투표
            else:
                votes[node_id] = random.choice(CANDIDATES)
        
        winner = sim.run_round(round_num=i + 1, votes_cast=votes)
